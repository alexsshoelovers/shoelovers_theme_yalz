...
function mvp_scripts_method() {
    ...
    wp_register_script('jqdfp', get_template_directory_uri() . '/js/jquery.dfp.min.js', array('jquery'), '', true);

	wp_enqueue_script('jquery');
    wp_enqueue_script('jqdfp');
	...
	$mvp_respond = get_option('mvp_respond'); if ($mvp_respond == "true") {
        if (isset($mvp_respond)) {
           $src =  get_template_directory_uri() . '/css/media-queries.css';
           $deps = '';
           $ver = filemtime( get_template_directory() . '/css/media-queries.css');
           $media = '';
	       wp_enqueue_style( 'media-queries', $src, $deps,$ver, $media );
	    }
    }
...
function excerpt($limit) {
  ...
  $excerpt = explode(' ', wp_trim_excerpt(), $limit);
    ...
    if ($tags) {
        ...
        if( $my_query->have_posts() ) { ?>
            <div id="related-posts">
            	<h4 class="post-header"><span class="post-header"><?php _e( 'TAMBIÉN TE PUEDE GUSTAR...', 'mvp-text' ); ?></span></h4>
			<ul>
...
function m_add_recirculation_ad ($instant_article) {
    ...
    $instant_article->addMetaProperty('fb:op-recirculation-ads', 'placement_id=1599480390370213_1847227412262175');

    $header = $instant_article->getHeader();
    $ads = $header->getAds();
    $ads = [];
    $inline =
            '<iframe src="http://pubads.g.doubleclick.net/gampad/adx?iu=/118157277/sl3_i_i_1_premium_300x250_54&sz=300x250&c=12345&ord=1747338796&tile=1&t=tag%3D'. basename($instant_article->getCanonicalURL()) .'" height="250" width="300"></iframe>' ;
    $ad =Ad::create()
        ->withHTML($inline)
        ->withHeight(250)
        ->withWidth(300);
